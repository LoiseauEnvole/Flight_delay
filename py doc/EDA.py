...
airports = spark.sql("select * from dscc202_group09_db.airlines_tmp")
display(airports)
...
#SCALE ISSUE DOMINATED
dep_num=[f.name for f in airlines.schema.fields if isinstance(f.dataType, DoubleType)]
df_num = airlines.filter(col("UNI_ID").like("%JFK%")).select(dep_num).toPandas()
plot_corr(df_num)

# COMMAND ----------

## WEATHER ISSUE DOMINATED
dep_num=[f.name for f in airlines.schema.fields if isinstance(f.dataType, DoubleType)]
df_num = airlines.filter(col("UNI_ID").like("%ORD%")).select(dep_num).toPandas()
plot_corr(df_num)
